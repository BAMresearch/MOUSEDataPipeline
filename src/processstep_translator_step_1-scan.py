...
from checkers import processing_possible_scan
...
def can_run(dir_path: Path, defaults: DefaultsCarrier, logbook_reader: Logbook2MouseReader, logger: logging.Logger) -> bool:
    ...
    """
    if not processing_possible_scan(dir_path):
        logger.info(f"Step 1 translation not possible for {dir_path}, required files missing... check DEBUG for details")
        logger.debug(f"Required files missing for step 1 translation not possible for {dir_path}. The following were not found: {processing_possible_scan(dir_path, return_list=True)}")
        return False

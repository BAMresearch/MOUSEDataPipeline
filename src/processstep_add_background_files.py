...
def run(dir_path: Path, defaults: DefaultsCarrier, logbook_reader: LogbookReaderLike | None, logger: logging.Logger):
    ...
    try:
        ...
        with h5py.File(input_file, "a") as h5f:
            bg_file_dataset = h5f.require_dataset(
                "/entry1/processing_required_metadata/background_file", shape=(), dtype=h5py.special_dtype(vlen=str)
            )
            rel_destination = ""
            if bg_id:
            ...
            bg_file_dataset.attrs.update(
                {"note": "background filename added by MOUSEDataPipeline, relative to preprocessed, stacked, file"}
            )
            # bg_file_dataset[...] = str(Path('..', '..') / bg_id[0:4] / bg_id[0:8] / f'MOUSE_{bg_id}_{config}_stacked.nxs') if bg_id else ''
            dbg_file_dataset = h5f.require_dataset(
            ...
            dbg_file_dataset.attrs.update(
                {
                    "note": "dispersed background filename added by MOUSEDataPipeline, relative to preprocessed, stacked, file"
                }
            )
        ...
        logger.info("Processstep processstep_add_background_files failed with stderr:")
        logger.info(e)

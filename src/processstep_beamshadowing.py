...
def run(dir_path: Path, defaults: DefaultsCarrier, logbook_reader: Logbook2MouseReader, logger: logging.Logger):
    ...
    try:
        ...
        pto_file = defaults.post_translation_dir / 'post_translation_operation_MOUSE_beamshadowing.py'
        cmd1 = [
            'python3', str(pto_file),
            '-f', str(input_file),
            # '-v', 
            '-k', 'roi_size=10', 'image_type=direct_beam',
        ]
        cmd2 = [
            'python3', str(pto_file),
            '-f', str(input_file),
            # '-v', 
            '-k', 'roi_size=10', 'image_type=sample_beam',
        ]
